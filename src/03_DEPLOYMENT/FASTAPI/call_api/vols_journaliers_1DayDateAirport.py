...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python vols_journaliers_1DayDateAirport.py YYYY-MM-DD AIRPORT_IATA",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if "nombre_departures" in df.columns and "nombre_arrivals" in df.columns:
        df["somme_nombre_departs_arrivees"] = (
            df["nombre_departures"].fillna(0).astype(int)
            + df["nombre_arrivals"].fillna(0).astype(int)
        )
    ...
    output_file = os.path.join(
        output_dir,
        f"vols_journaliers_{airport_code}_{from_dt.date()}.csv"
    )
