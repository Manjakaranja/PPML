...
def main():
    ...
    if iata_input in iata_to_airports:
        icao = iata_to_airports[iata_input]
    else:
        icao = iata_input
    ...
    # Filtrer pour ne garder que la date demandée
    mask = df["time"].dt.strftime("%Y-%m-%d") == forecast_start_str
    df = df[mask].sort_values(["icao", "time"]).reset_index(drop=True)
    # Harmonisation finale : remplacer LYON par LYS dans la colonne icao
