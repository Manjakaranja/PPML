...
def main() -> int:
    ...
    print(f"Période demandée : {from_dt.strftime('%Y-%m-%d %H:%M')} -> {to_dt.strftime('%Y-%m-%d %H:%M')}")
    print("Récupération des vols AeroDataBox (7 jours à venir)...")
    ...
    for airport_code in ALL_AIRPORTS:
        ...
        for start, end in daterange_12h(from_dt, to_dt):
            ...
            try:
                ...
                # Ajout d'une ligne vide pour ce créneau/airport
                empty_row = {col: None for col in reorder_columns(pd.DataFrame()).columns}
                empty_row["airport_origin"] = airport_code
                empty_row["scheduled_departure"] = start_str
                empty_row["scheduled_arrival"] = end_str
            ...
            # Si aucune donnée trouvée pour ce créneau/airport, ajouter une ligne vide
            if not found_data:
                empty_row = {col: None for col in reorder_columns(pd.DataFrame()).columns}
                empty_row["airport_origin"] = airport_code
                empty_row["scheduled_departure"] = start_str
    ...
    tls_log_path = os.path.join(os.getcwd(), "tls_interrogations_7days.csv")
    pd.DataFrame(tls_logs, columns=tls_log_columns).to_csv(
        tls_log_path,
        index=False,
        encoding="utf-8-sig"
    )
    print(f"[INFO] Fichier tls_interrogations_7days.csv généré à : {tls_log_path}")
    ...
    output_file = "flights_paris_regional_future.csv"
    ...
    congestion_cible_path = os.path.join(output_dir, "congestion_cible_future.csv")
    congestion_france_path = os.path.join(output_dir, "congestion_France_future.csv")
    congestion_ww_path = os.path.join(output_dir, "congestion_WW_future.csv")
