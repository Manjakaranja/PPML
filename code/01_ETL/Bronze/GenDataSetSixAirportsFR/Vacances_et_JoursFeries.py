...
def main():
    ...
    # Début = aujourd'hui - 364 jours, Fin = aujourd'hui
    end = datetime.today().date()
    start = end - timedelta(days=364)

    output_hist = os.path.join(output_dir, "histo_calendrier_jferies_et_vacances.csv")
    ...
    output__future = os.path.join(output_dir, "future_calendrier_jferies_et_vacances.csv")
    start__future = end + timedelta(days=1)
    end__future = end + timedelta(days=7)
    with open(output__future, mode="w", newline="", encoding="utf-8-sig") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, delimiter=";")
        writer.writeheader()
        for d in daterange(start__future, end__future):
            city_values = {}
            vacation_cities = []
            vacation_labels = []
            for city in cities:
            ...
            for city in cities:
                row[city_col_map[city]] = city_values[city_col_map[city]]
            writer.writerow(row)
    print(f"Fichier généré : {output__future}")
