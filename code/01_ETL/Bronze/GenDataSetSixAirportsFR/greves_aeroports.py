...
def main():
    import os
    # Crée le dossier OutputDataGreves s'il n'existe pas
    output_dir = "OutputDataGreves"
    os.makedirs(output_dir, exist_ok=True)
    # Supprime tous les fichiers du dossier OutputDataGreves
    ...
    rows = build_rows()
    output_hist = os.path.join(output_dir, OUTPUT_CSV)
    write_csv(rows, output_hist)

    nb_yes = 0
    for row in rows:
        for airport_code in AIRPORTS:
            if row[f"GREVE_{airport_code}"] == "Oui":
                nb_yes += 1

    print(f"[OK] CSV généré : {output_hist}")
    print(f"[OK] Nombre total de couples (jour, aéroport) marqués 'Oui' : {nb_yes}")
    ...
    for offset in range(1, 8):
        ...
        for airport_code in AIRPORTS:
            greve_annoncee = False
            label = ""
            if greve_annoncee:
                row[f"GREVE_{airport_code}"] = "Oui"
                row[f"LABEL_{airport_code}"] = label
            else:
                row[f"GREVE_{airport_code}"] = "Non"
                row[f"LABEL_{airport_code}"] = ""
        future_rows_7days.append(row)
    output_7days = os.path.join(output_dir, "greve_aeroports_future.csv")
    write_csv(future_rows_7days, output_7days)
    print(f"[OK] CSV généré : {output_7days}")
